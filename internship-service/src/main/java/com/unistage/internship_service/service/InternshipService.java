...
public class InternshipService {
  ...
  private final ObjectMapper objectMapper; // 💡 Injected to transform records into JSON plain-text
  ...
  public Optional<InternshipOffer> getOfferById(Long id) {
  ...
  private void sendNotification(Long userId, String title, String message) {
    try {
      // Stringify the payload event to map nicely with the consumer's text properties
      String jsonPayload = objectMapper.writeValueAsString(new NotificationEvent(userId, title, message));

      kafkaTemplate.send("notification-topic", jsonPayload).get();
      System.out.println("✅ Kafka successfully confirmed receipt of notification event.");
    } catch (Exception e) {
      System.err.println("❌ CRITICAL KAFKA ERROR: " + e.getMessage());
      e.printStackTrace();
    }
  }
}
